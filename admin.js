document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.loadReportsLog = async function() {
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding: 20px;">Fetching user reports...</td></tr>';
        
        try {
            // 1. Fetch raw reports from user_reports table
            const { data: reports, error } = await supabase.from('user_reports').select('*').order('created_at', { ascending: false });
            
            if (error) {
                if (error.code === '42P01') {
                     tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:var(--text-secondary); padding: 20px;">The <code>user_reports</code> table does not exist yet. Please run the SQL setup script in Supabase!</td></tr>';
                } else {
                     throw error;
                }
                return;
            ...
            if (!reports || reports.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color:var(--text-secondary); padding: 20px;">No user reports lodged yet.</td></tr>';
                return;
            }

            // 2. Batch Fetch Reporters & Reported Users Profiles
            const userIds = new Set();
            reports.forEach(r => {
                 if (r.reporter_id) userIds.add(r.reporter_id);
                 if (r.reported_uid) userIds.add(r.reported_uid);
            });
            const profileMap = {};
            if (userIds.size > 0) {
                try {
                    const { data: profiles } = await supabase.from('profiles').select('id, username, avatar_url').in('id', Array.from(userIds));
            ...
            reports.forEach(report => {
                const reporter = profileMap[report.reporter_id] || { username: 'Unknown (' + String(report.reporter_id).substring(0,6) + ')' };
                const reported = profileMap[report.reported_uid] || { username: 'Unknown (' + String(report.reported_uid).substring(0,6) + ')' };
                
                // Action Buttons
                const actionHtml = `
                    <button class="action-btn report-ban-btn" data-uid="${report.reported_uid}" data-rid="${report.id}" style="color:var(--danger); border-color:var(--danger); margin-right: 8px;">Ban User</button>
                    <button class="action-btn report-dismiss-btn" data-rid="${report.id}" style="color:var(--text-secondary); border-color:var(--border-color);">Dismiss</button>
                `;

                tbody.insertAdjacentHTML('beforeend', `
                    <tr id="report-row-${report.id}">
                        <td style="font-size:12px; color:var(--text-secondary);">${String(report.id).substring(0,8)}</td>
                        <td style="font-weight: 600;">@${reporter.username}</td>
                        <td style="font-weight: 600; color:var(--danger);">@${reported.username}</td>
                        <td style="max-width:300px; white-space:pre-wrap;">${report.reason || '<em>No reason provided</em>'}</td>
            ...
            document.querySelectorAll('.report-ban-btn').forEach(btn => {
                btn.addEventListener('click', async function() {
                    const reportedUid = this.dataset.uid;
                    const reportId = this.dataset.rid;
                    
                    if(confirm("Are you sure you want to permanently BAN this user?")) {
                        const originalText = this.textContent;
                        this.textContent = 'Banning...';
                        try {
                            const { error: banErr } = await supabase.from('profiles').update({ is_banned: true }).eq('id', reportedUid);
                            if(banErr) throw banErr;
                            
                            // Mark report as "dismissed" automatically
                            await supabase.from('user_reports').delete().eq('id', reportId);
                            
                            const row = document.getElementById('report-row-' + reportId);
                            if(row) { 
                                row.style.opacity = '0.3'; 
                                row.innerHTML = '<td colspan="6" style="text-align:center; color:var(--danger); font-weight:600;">User Banned & Report Cleared</td>';
                                setTimeout(() => row.remove(), 2000); 
                            }
                        } catch(e) {
                            alert("Database Error: Could not ban user. Please ensure the 'is_banned' column is created on the profiles table.");
                            this.textContent = originalText;
                        }
                    }
                });
            ...
            document.querySelectorAll('.report-dismiss-btn').forEach(btn => {
                btn.addEventListener('click', async function() {
                    const reportId = this.dataset.rid;
                    this.textContent = 'Dismissing...';
                    try {
                        await supabase.from('user_reports').delete().eq('id', reportId);
                        const row = document.getElementById('report-row-' + reportId);
                        if(row) row.remove();
            ...
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; color: var(--danger);">Global schema sync failure.</td></tr>';
        }
    };
