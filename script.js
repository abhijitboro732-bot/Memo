document.addEventListener('DOMContentLoaded', async () => {
    ...
    window.loadReelsView = async function(initialPostId = null) {
        ...
        let recommendedReels = await getRecommendedPosts(true);
        
        if (initialPostId) {
             const targetIdx = recommendedReels.findIndex(r => String(r.id) === String(initialPostId));
             if (targetIdx !== -1) {
                  const targetReel = recommendedReels.splice(targetIdx, 1)[0];
                  recommendedReels.unshift(targetReel);
             } else {
                  // Fallback: If AI recommendation missed it (since getRecommendedPosts has a limit),
                  // fetch the specific actual post and unshift it so it definitely shows.
                  try {
                      const { data } = await supabase.from('posts').select('*').eq('id', initialPostId).single();
                      if(data) recommendedReels.unshift(data);
                  } catch(e) { }
             }
        }
        ...
        recommendedReels.forEach(post => {
            const html = `
                <div class="reel-item" data-id="${post.id}">
                    <video src="${post.image_url}#t=0.1" preload="metadata" class="reel-video" autoplay loop muted playsinline></video>
