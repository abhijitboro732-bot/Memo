document.addEventListener('DOMContentLoaded', async () => {
                ...
                const reelHTML = `
                    <div class="reel-container" data-id="${post.id}" style="height: calc(100vh - 40px); width: 100%; scroll-snap-align: start; background: #000; border-radius: 8px; position: relative; overflow: hidden; display: flex; justify-content: center; align-items: center; margin-bottom: 0;">
                        ...
                        <div style="position: absolute; bottom: 20px; left: 20px; right: 80px; color: white; text-shadow: 0 1px 4px rgba(0,0,0,0.8); z-index: 10;">
                            ...
                            <div class="reel-comment-section" style="display: none; flex-direction: column; width: 100%; background: rgba(0,0,0,0.6); backdrop-filter: blur(4px); padding: 12px; border-radius: 12px; margin-top: 8px; pointer-events: auto;">
                                <div class="reel-comments-list" style="max-height: 200px; overflow-y: auto; font-size: 13px; margin-bottom: 12px; display: flex; flex-direction: column; gap: 8px;"></div>
                                <div style="display: flex; gap: 8px;">
                                    <input type="text" class="reel-comment-input" placeholder="Add comment..." style="flex-grow: 1; padding: 8px 12px; border-radius: 20px; border: none; outline: none; background: rgba(255,255,255,0.2); color: white;">
                                    <button class="reel-post-comment-btn" style="background: transparent; color: var(--color-accent); border: none; font-weight: 600; cursor: pointer;">Post</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <article class="post" data-id="${post.id}">
                        ...
                        <div class="post-comments">
                            ...
                            <div class="add-comment">
                                ...
                                <button class="post-comment-btn" style="opacity: 1; cursor: pointer;">Post</button>
                            </div>
                        </div>
                    </article>
                `;
    ...
    function attachPostInteractions() {
        ...
        document.querySelectorAll('.post-comment-btn').forEach(btn => {
            btn.addEventListener('click', async function() {
                ...
                const newCommentHtml = `<div style="font-size: 14px; margin-bottom: 4px;"><span class="username" style="font-weight:600; font-size:14px; margin-right:4px;">${username}</span>${text}</div>`;
                commentsList.insertAdjacentHTML('beforeend', newCommentHtml);
                
                // Upload to Supabase (assuming a comments table exists)
                try {
    ...
    async function loadAllComments() {
        ...
        try {
            ...
            if(error) return; // Silent fail if table doesn't exist yet
            
            if(comments && comments.length > 0) {
                comments.forEach(c => {
                    const postEl = document.querySelector(`.post[data-id="${c.post_id}"]`);
                    if(postEl) {
