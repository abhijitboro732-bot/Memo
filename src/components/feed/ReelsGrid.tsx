...
import { fetchRecommendedVideos, saveVideo, unsaveVideo, getSavedVideoIds } from '@/lib/supabase/videos';
import { trackUserBehavior } from '@/lib/supabase/interactions';
import Avatar from '../ui/Avatar';
import SuggestedStoriesRow from './SuggestedStoriesRow';
  ...
  useEffect(() => {
    if (prevIndexRef.current !== activeIndex) {
      ...
      if (prevVideo && user && prevVideo.id) {
        const type = watchTimeSeconds < 2 ? 'skip' : 'view';
        const weight = watchTimeSeconds < 2 ? -1 : Math.min(5, watchTimeSeconds / 5);
...
export default function ReelsGrid() {
  ...
  // Fetch AI recommended videos from Supabase and merge with mock data
  useEffect(() => {
  ...
  return (
    <>
      <div className="reels-grid-wrapper">
        ...
        {/* Desktop Suggested Accounts Row */}
        <SuggestedStoriesRow />
