...
export default function StoryBar() {
  const [suggestedUsers, setSuggestedUsers] = useState<User[]>(MOCK_USERS.slice(0, 4));
  ...
  useEffect(() => {
    fetchRealUsers(10).then(users => {
      if (users.length > 0) {
        // Add mock users to fill if needed
        const seen = new Set(users.map(u => u.id));
        const merged = [...users, ...MOCK_USERS.filter(u => !seen.has(u.id)).slice(0, 4)];
  ...
  return (
    <div className="story-bar">
      ...
      {/* Suggested — Real users from posts table */}
      <div className="sb-section">
        <div className="sb-section-label">
          <span>Suggested</span>
        </div>
          ...
          {suggestedUsers.map((user) => (
            ...
            >
              <motion.div
                ...
                whileTap={{ scale: 0.95 }}
              >
                <div className={`sb-avatar-ring ${user.is_live ? 'live' : ''}`}>
                  <img
                    src={user.avatar_url}
                    alt={user.username}
                    className="sb-avatar-img"
                ...
                <span className="sb-avatar-name">{user.username.length > 8 ? user.username.slice(0, 8) + '…' : user.username}</span>
                {user.is_verified && <span className="sb-verified-dot" />}
              </motion.div>
            </Link>
          ))}
        </ScrollableRow>
