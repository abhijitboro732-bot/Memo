...
import { X, Send, Search, Check, Loader2, Share2, Link2, MessageSquare } from 'lucide-react';
      ...
      {isOpen && (
        <>
          ...
          >
            ...
            <div className="flex items-start gap-4 p-4 overflow-x-auto no-scrollbar border-b border-gray-50 bg-gray-50/50">
              {/* Native Share / Apps */}
              <button
                onClick={() => {
                  if (navigator.share && video) {
                    navigator.share({
                      title: `Watch ${video.user.display_name}'s video on Skipsee`,
                      text: video.caption,
              ...
              >
                <div className="w-12 h-12 rounded-full bg-purple-100 flex items-center justify-center border border-purple-200 shadow-sm active:scale-95 transition-transform">
                  <Share2 size={20} className="text-purple-600" />
                </div>
                <span className="text-[11px] font-medium text-gray-700">Share via...</span>
              </button>
              ...
              <button
                ...
                className="flex flex-col items-center gap-1.5 shrink-0"
              >
                <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200 shadow-sm active:scale-95 transition-transform">
                  <Link2 size={20} className="text-gray-700" />
                </div>
                <span className="text-[11px] font-medium text-gray-700">Copy Link</span>
              </button>
              ...
              <button
                onClick={() => {
                  if (video && typeof window !== 'undefined') {
                    const url = encodeURIComponent(`${window.location.origin}/video/${video.id}`);
                    const text = encodeURIComponent(`Check out this video on Skipsee! `);
                    window.open(`https://wa.me/?text=${text}${url}`, '_blank');
                  }
                }}
                className="flex flex-col items-center gap-1.5 shrink-0"
              >
                <div className="w-12 h-12 rounded-full bg-green-100 flex items-center justify-center border border-green-200 shadow-sm active:scale-95 transition-transform">
